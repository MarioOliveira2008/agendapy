...
class Tarefa:
    ...
    def id(cls, id: int) -> Self:
        with Database() as db:
            query: str =("""
                         SELECT titulo_tarefa, data_conclusao,
                          concluida FROM tarefas WHERE id = ?;
        ...
        return cls(
            id_tarefa=id, titulo_tarefa=titulo, data_conclusao=data, concluida=concluida
        )
    ...
    def obter_tarefas(cls) -> list[Self]:
        with Database() as db:
            query: str = (
                "SELECT titulo_tarefa, data_conclusao, concluida, id FROM tarefas;"
            )
            resultados: list[Any] = db.buscar_tudo(query)
            tarefas: list[Self] = [
                cls(titulo, data, concluida, id)
                for titulo, data, concluida, id in resultados
            ]
            return tarefas
